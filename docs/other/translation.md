...
- 翻译完成后把 FMHY 主站链接改写为 `pingti.org`；API、图片和搜索等 FMHY 子域名保持不变。
...
4. 把新增内容中指向 FMHY 主站的链接改写为 `https://pingti.org`。
5. 完成后创建或更新本地化 Draft PR。
6. 余额不足时保留分支和断点，等待充值后继续。
